...
\section{Especificação do Problema e dos Requisitos de Dados}
...
Através do sistema, é possível que o aluno tenha acesso à um banco de questões fornecido pelo professor de uma determinada disciplina, e possa estudar através deste para provas e outras avaliações. O professor também consegue ter um controle do desempenho do aluno ao longo do semestre.

% \subsection{Especificação do Problema}

Nas subseções conseguintes abordaremos as características, os atributos e os comportamentos das entidades envolvidas no sistema.
...
O objetivo do projeto conceitual, descrito na seção anterior, é mostrar como o mini-mundo descrito pode ser representado em uma abordagem de Banco de Dados. Como já o descrevemos na seção anterior, apresentaremos a seguir as Tabelas de Metadados e o Modelo Entidade Relacionamento (MER) completo.\\

A tabela de metadados contém basicamente um resumo de cada tipo-entidade, com seu nome e atributos, junto com o tipo e restrição de cada atributo. Já o MER é um diagrama que apresenta todos os tipos-entidade e todos os tipos-relacionamento, junto com os atributos de cada tipo-entidade (identificados como simples, chave primária ou chave parcial), e as cardinalidades de cada tipo-relacionamento.\\
...
\begin{table}[h]
  ...
  \caption{Tipo-Entidade Disciplina}
    ...
    \multicolumn{2}{|c|}{\textbf{Tipo-Entidade}} & \textbf{Atributo} & \textbf{Tipo} & \textbf{Restrição} \\\hline
    \midrule
    \multicolumn{2}{|c|}{\multirow{\texttt{Disciplina}}} & \texttt{CodigoDisciplina} & Identificador & Obrigatório \\
